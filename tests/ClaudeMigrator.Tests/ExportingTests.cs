...
public sealed class ExportingTests
{
    ...
    public void LocalBundleExporterCopiesProfileAndRestoresTargets()
    {
        ...
        var restoreHome = Path.Combine(workspace.Root, "restore_home");
        Directory.CreateDirectory(restoreHome);
        var restoreResult = exporter.RestoreLocalBundle(result.ZipPath, destinationHome: restoreHome, targetApps: new[] { "claude", "codex" });
        ...
        Assert.True(File.Exists(Path.Combine(restoreHome, ".claude", "CLAUDE.md")));
        Assert.True(File.Exists(Path.Combine(restoreHome, ".claude", "projects", "alpha", "notes.txt")));
        Assert.True(File.Exists(Path.Combine(restoreHome, ".claude.json")));
        Assert.True(File.Exists(Path.Combine(restoreHome, ".codex", "CLAUDE.md")));
    }
